...
name_source_1 <- sources %>% filter(grepl('ITIS', title)) %>% pull(title)
id_source_1 <- sources %>% filter(grepl('ITIS', title)) %>% pull(id)

name_source_2 <- sources %>% filter(grepl('IUCN Red List', title)) %>% pull(title)
id_source_2 <- sources %>% filter(grepl('IUCN Red List', title)) %>% pull(id)

sources <- tibble(source= c(1,2),
...
check_name <- function(species, sources){
  species_checked <- tibble(previousIdentification = character(),
  ...
  for(sp in species) {
    ...
    if (nrow(sp_check_1)==0){
      ...
      if (nrow(sp_check_2)==0){
        sp_checked <- tibble(previousIdentification = sp,
                             scientificName = NA,
                             remarks = str_glue('NOT FOUND in {sources$name[1]} or {sources$name[2]}'))
        ...
        sp_checked <- tibble(previousIdentification = sp,
                             ...
                             remarks = str_glue('Updated according to {sources$name[2]}'))
        species_checked <- rbind(species_checked, sp_checked)
      }
      else{
        sp_checked <- tibble(previousIdentification = sp,
                             scientificName = sp,
                             remarks = str_glue('Ok ({sources$name[2]})'))
        species_checked <- bind_rows(species_checked, sp_checked)
      }
    } 
    else if (nrow(sp_check_1)!=0 && sp!=sp_check_1$matched_name2[1]){
      sp_checked <- tibble(previousIdentification = sp,
                           scientificName = sp_check_1$matched_name2[1],
                           remarks = str_glue('Updated according to {sources$name[1]}'))
      species_checked <- bind_rows(species_checked, sp_checked)
    } 
    else {
      sp_checked <- tibble(previousIdentification = sp,
                           scientificName = sp,
                           remarks = str_glue('Ok ({sources$name[1]})'))
      species_checked <- bind_rows(species_checked, sp_checked)
    }
  }
...
# # A tibble: 3 × 3
#   previousIdentification  scientificName  remarks                                                         
#   <chr>                   <chr>           <glue>                                                          
...
mammlas_species_check <- check_name(mammals$Species, sources)
