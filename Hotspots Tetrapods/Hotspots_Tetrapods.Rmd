...
Each table has a GridID and the values for the 4 variables NR, SR, TP and ER for each grid. For instance, the table for Amphibia at 50x50km scale looks like this:
```{r echo=FALSE}
Amphibia_50.GRIDs
```
...
Let's calculate the SACs using the data of birds of Uruguay, for the grid-cell size of 25x25km. To run the function we need to have a list of grid-cells, sample numbers and species. This is, a list of species recorded for each grid-cell. I have created a function in Python to extract this list of samples/species per grid cell, check it here: (abundances.py)[]
